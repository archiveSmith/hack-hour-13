...
function getPINs(observed) {
  // get all potential nums for each input
    // get +3, -3, +1, -1 and if 8 get 0
    // only those between 0 and 9
  const combos = []
  for (let i = 0; i < observed.length; i += 1) {
    let num = Number(observed[i]);
    let left = num-1 % 3 !== 0 && num !== 0 ? num - 1 : null
    let right = num % 3 !== 0 && num !== 0 ? num + 1 : null
    let above = num > 3 ? num - 3 : null
    if (num === 0) above = 8
    let below = num < 7 ? num + 3 : null
    if (num === 8) below = 0
    const combo = [];
    [left, right, above, below].forEach((item) => {if (item !== null) combo.push(item)})
    combos.push(combo);
  }
  console.log(combos);
}
