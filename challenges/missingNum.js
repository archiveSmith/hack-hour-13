...
function missingNum(arr) {
  let arrSum = arr.reduce((acc, initVal) => {return acc += initVal})
  let min = arr.reduce((acc, curr )=> {return Math.min(acc, curr)});
  let expectedSum = 0
  let i = 0; 
  while(i <= arr.length){
    expectedSum += min 
...
console.log(missingNum([2,3,1,5,4, 6, 7, 8, 9, 11,12,14,13])) // 10;
