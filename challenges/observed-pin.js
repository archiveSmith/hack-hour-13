...
function getPINs(observed) {
  const pin = observed.split('').map(d => Number(d));
  const adjNums = {
    1: [2, 4],
    2: [1, 3, 5],
    3: [2, 6],
  ...
  (function findCombinations(arr) {
    ...
    arr.forEach((d, i) => {
      adjNums[d].forEach((n) => {
        arr[i] = n;
        const newArr = arr.slice(0, i + 1).concat(arr.slice(i + 1));
        const pushArr = pin.slice(0, pin.length - newArr.length).concat(newArr);
        result.push(pushArr);
        findCombinations(arr.slice(i + 1));
      });
    });
  }(pin));
