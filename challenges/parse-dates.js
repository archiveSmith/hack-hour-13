...
function parseWeekday(str, currDate) {
  const match = str.match(/^(\w+) (\d{1,2}):(\d{2}) ^(\w{2})/);



  if (match) {
    // map parsed weekday to int value
    const weekdayToNum = {
      Sunday: 0,
      Monday: 1,
      Tuesday: 2,
...
function parseDates(datetimeStr) {
  // current date to alter
  const currDate = new Date();

  // weekday: return last date before today


  // day and month

  // invalid: current date and time
}
