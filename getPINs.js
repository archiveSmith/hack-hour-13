...
function getPINs(observed) {
  ...
  if (Number(observed[i]) === 1 || Number(observed[i]) === 2 || Number(observed[i]) === 3) {
    if (Number(observed[i]) === 1) {
      let right = Number(observed[i] + 1);
      let below = Number(observed[i] + 3);
      res[i].push(toString(right), toString(below));
    } else if (Number(observed[i]) === 3) {
      let left = Number(observed[i] - 1);
      let below = Number(observed[i] + 3);
      res[i].push(toString(left), toString(below));
    }
    let below = Number(observed[i] + 3);
    let left = Number(observed[i] - 1);
    let right = Number(observed[i] + 1);
    res[i].push(toString(below), toString(left), toString(right));
  }
  else if (Number(observed[i]) === 7 || Number(observed[i]) === 8 || Number(observed[i]) === 9) {
    if (Number(observed[i]) === 7) {
      let right = Number(observed[i] + 1);
      let above = Number(observed[i] - 3);
      res[i].push(toString(right), toString(above));
    } else if (Number(observed[i]) === 9) {
